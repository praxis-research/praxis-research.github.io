...
def build():
  ...
  <div class="note">Qwen3-14B after install; AuditBench is four quirks, false facts five claims. Each radar overlays the unmodified model (dashed), native and graft, on the same nine axes: expression at the top (open-ended belief, installed belief), coherence on the right, capabilities on the left. For AuditBench, open-ended belief is the judged rubric on open-ended elicitation. False facts has no known-fiction margin (its reality gap is real vs never-seen fakes), so that axis is empty. Both arms install the belief. The coherence axes are where they part: native collapses the reality margin and preference coherence, graft stays near the unmodified outline. Capabilities roughly level. Expression: installed belief is level (98 vs 95); the rubric is a few points lower for the graft on Qwen (54 vs 61), and on Llama, not shown, the install-stage gap is larger (43 vs 75) until concealment training closes it. On false facts we matched the native checkpoint to the graft's belief level, so the coherence gap there is at equal installation. (~60 s)</div>
</section>''')
